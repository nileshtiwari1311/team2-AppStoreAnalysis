...
{
    system(("node google_play_store_reviews.js "+ appID  + " > result_reviews_2.txt 2>&1").c_str());
...
{
	string empty = "";
    system(("python senti_score_google.py "+ empty  + " > python_score.txt 2>&1").c_str());
}
...
int main( int argc ,char * argv[] )
{
    ...
    final_score.open("google_senti.txt");
    ifstream app_list;
    string senti_type = argv[1];
    app_list.open("output.txt");
    ...
    for(int k=0; k<appIdList.size(); k++)
    {
	    ...
	    app_details.open("output_google_text_reviews.txt");
